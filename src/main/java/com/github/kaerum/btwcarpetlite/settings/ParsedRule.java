...
public final class ParsedRule<T> implements Comparable<ParsedRule> {
    ...
    {
        if (BTWCarpetLiteServer.settingsManager != null && BTWCarpetLiteServer.settingsManager.locked)
            return null;
    ...
    ParsedRule<T> set(ServerCommandSource source, T value, String stringValue)
    {
        try
        {
            ...
            if (!value.equals(get()))
            {
                ...
                BTWCarpetLiteServer.settingsManager.notifyRuleChanged(source, this, stringValue);
            }
        }
